...
import json
from src.Synthetic_document_pipeline.IA_Models.Document_Layout_Generator_Models.LayoutTransformer.utils.utils_predictor import trim_tokens, gen_colors

#https://deepai.org/machine-learning-glossary-and-terms/padding#:~:text=Padding%20is%20the%20process%20of,to%20the%20desired%20output%20dimensions.
#https://stackoverflow.com/questions/64015076/how-can-i-assign-a-list-to-a-torch-tensor
...
class JSONLayout(Dataset):
    ...
    def get_bbox_coords(self, layout):
        layout = layout.reshape(-1)
        layout = trim_tokens(layout, self.bos_token, self.eos_token, self.pad_token)
        layout = layout[: len(layout) // 5 * 5].reshape(-1, 5)
        #print(layout)
        ...
        hold_coords_per_layout = []
        for i in range(len(layout)):
            x1, y1, x2, y2 = box[i]
            bbox = [float(x1), float(y1),float( x2), float(y2)]
            #print(bbox)
            cat = layout[i][0]
            categorie_dict = {"text": 256, 
                            "title": 257, 
                            "list": 258, 
                            "table": 259, 
                            "figure": 260}
            for k, v in categorie_dict.items():
                if v == cat:
                    cat = k 
            temp_list = [bbox, cat]
            #print(temp_list)
            hold_coords_per_layout.append(temp_list)
        #print(hold_coords_per_layout)

        return hold_coords_per_layout
