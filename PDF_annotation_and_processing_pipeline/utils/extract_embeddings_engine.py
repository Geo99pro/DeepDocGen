...
class ExtractEmbeddingsEngine:
    ...
    def __init__(self, dataset, model, batch_size: int, shuffle_dataloader: bool, save_embedding_extracted: bool, device: str, **kwargs):
    ...
    def extract_embeddings(self):
        ...
        with torch.no_grad():
            for image, name in tqdm(self.data_loader):
                ...
                # else:
                #     embedding_extracted = self.model(image).squeeze().detach().numpy()
        ...
        if self.save_embedding_extracted:
            embeddings_manager = EmbeddingsManagerEngine(f'{self.kwargs.get("path_to_save_embeddings")}embeddings_extracted_with_model', embedding_extracted_dict)
