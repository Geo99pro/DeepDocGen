...
class ReduceDimensionEngine:
    ...
    def reduce_dimension_with_tSNE(self,
                                ...
                                t_SNE_init: str,
                                random_state: int,
                                path_to_save_div_vs_perp: str,
                                should_save_best_embeddings: bool,
                                **kwargs)-> np.ndarray:
        """
        This method reduces the dimension of the data to 2D space and returns the best embeddings reduced.
        ...
        file_path = os.path.join(path_to_save_div_vs_perp, 'perplexity_vs_divergence.png')
        fig.write_image(file_path)

        if should_save_best_embeddings:
            best_embedding_reduced = hold_meta_data[np.argmin(divergence_list)][0]['embeddings']
            embedding_manager = EmbeddingsManagerEngine(path= f'{kwargs.get("path_to_save_best_embeddings")}best_embeddings_reduced', element = best_embedding_reduced)
            embedding_manager.save_embeddings()
            return best_embedding_reduced
            #embedding_manager = EmbeddingsManagerEngine(path= kwargs.get('path_to_save_metadata'), element = hold_meta_data) # decoment this line if you want to save the metadata
            #EmbeddingsManagerEngine(path= kwargs.get('path_to_save_divergence'), element = divergence_list).save_embeddings() # decoment this line if you want to save the divergence
