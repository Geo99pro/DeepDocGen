...
class DataFrameEngine:
    ...
    Methods:
        ...
        test_cosine_similarity(meta_data_folder): Analyzes cosine similarity within each cluster.
        plot_distribution_of_PDF_pages_by_cluster(): Generates a distribution plot for clusters.
        setup_folder(): Prepares a folder for saving files.
        copy_image(): Copies images based on similarity thresholds.
    """
    ...
    def create_dataframe(self):
        embeddings_df = pd.DataFrame(self.best_reduced_embeddings, 
                                     columns=[self.desired_columns_names + str(i) for i in range(self.best_reduced_embeddings.shape[1])])
        ...
        if self.save_to_excel_format:
            output_path = os.path.join(self.kwargs.get("path_to_save_dataframe"), 'embeddings_dataframe.xlsx')
    ...
    def test_cosine_similarity(self, meta_data_folder: dict):
        ...
        for cluster_label in np.unique(self.kmeans_labels):
            ...
            save_path = self.kwargs.get("path_to_save_dataframe")
    ...
    def plot_distribution_of_PDF_pages_by_cluster(self, mean_similarity: pd.Series, overall_mean: float, cluster_label: int):
        ...
        plt.legend()
        plt.grid(axis='both')
        plt.savefig(os.path.join(self.kwargs.get("path_to_save_dataframe"), f'cluster_{cluster_label}_similarity_distribution.png'))
        plt.close()
